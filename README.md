# AI Auto Job Applier 🤖
This is an web scraping bot that automates the process of job applications on professional networking platforms. It searches for jobs relevant to you, answers all questions in application form, customizes your resume based on the collected job information, such as skills required, description, about company, etc. and applies to the job. Can apply 100+ jobs in less than 1 hour. 


## 📽️ See it in Action
Demo video available showing the automation process in action.
...
Setup tutorial video available for installation and configuration.

1. Python 3.10 or above. Download and install Python from the official website. **Please make sure Python is added to Path in System Environment Variables**.
2. Install necessary Undetected Chromedriver, PyAutoGUI and Setuptools packages. After Python is installed, OPEN a console/terminal or shell, Use below command that uses the pip command-line tool to install these 3 package.
  ...
  ```
3. Download and install latest version of Google Chrome in it's default location.
4. Clone the current git repo or download it as a zip file.
5. (Not needed if you set `stealth_mode = True` in `config/settings.py` ) Download and install the appropriate Chrome Driver for Google Chrome and paste it in the location Chrome was installed.
  <br> <br>
  ***OR*** 
  <br> <br>
...
6. If you have questions or need help setting it up or to talk in general, join the community server.
...
6. (Optional) Don't forget to add you default resume in the location you mentioned in `default_resume_path = "all resumes/default/resume.pdf"` given in `/config/questions.py`. If one is not provided, it will use your previous resume submitted in the platform or (In Development) generate custom resume if OpenAI APT key is provided!
7. Run `runAiBot.py` and see the magic happen, or run `run_dashboard.py` to launch the GUI dashboard for full control.
8. If you have questions or need help setting it up or to talk in general, join the community server.
...
- **Platform Policies**: Professional networking platforms have specific policies regarding web scraping and data collection. The responsibility to review and comply with these policies before engaging, interacting, or undertaking any actions with this program bears upon yourself. Be aware of the limitations and restrictions imposed by the platform to avoid any potential violation(s).
...
## 🐧 Contact
- **Email**    : Contact available in source code


## 🙌 Community Support and Discussions
- **Community Server** : Available for support and discussions
- **Repository**
    - All Discussions
    - Announcements
    - General
...
[back to the top](#ai-auto-job-applier-)
