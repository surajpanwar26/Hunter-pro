...
(function() {
    ...
    async function init() {
        ...
        try {
            const settingsResult = await chrome.storage.sync.get('universalAutoFillSettings');
            const settings = settingsResult.universalAutoFillSettings || {};
            
            if (settings.extensionEnabled === false) {
                log('Extension disabled in settings — content script will not activate');
                return;
            }
            
            // Apply detection mode from settings
            // 'universal' = detect all form fields, 'easy-apply' = only LinkedIn Easy Apply modals
            const detectionMode = settings.detectionMode || 'universal';
            log('Detection mode:', detectionMode);
            CONFIG.DETECTION_MODE = detectionMode;
        } catch (e) {
            log('Could not read extension settings, using defaults:', e);
        }
        ...
        // In 'easy-apply' mode, only observe for Easy Apply modals
        // In 'universal' mode, observe all DOM changes for any fillable forms
